...
habitBlueprint = Blueprint(
    "habitBlueprint", __name__, template_folder="pages", static_folder="static"
)
...
def createHabitDataRoute():
    if request.method == "POST":
        h = Habit(
            habit=request.form["habit"], dayGoal=request.form["dayGoal"], userID=session["userId"]
        )
        db.session.add(h)
        db.session.commit()
